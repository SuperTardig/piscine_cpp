...
Form::Form(const Form &init)
    : name(init.name), exe(init.exe), sign(init.sign), status(init.status) {
...
std::ostream &operator<<(std::ostream &os, const Form &form) {
  os << "The form name is " << form.getName()
     << ". The grade required to sign it is " << form.getSign()
     << " and the one to execute it is " << form.getExe()
     << ". The form status is " << form.getStatus();
  return os;
}
...
int Form::getSign() const { return this->sign; }

int Form::getExe() const { return this->exe; }

bool Form::getStatus() const { return this->status; }
...
void Form::beSigned(Bureaucrat &dude) {
  if (this->getStatus() == false) {
    ...
    cout << dude.getName()
         << " couldn't sign the form because it was already signed" << endl;
  return;
}
